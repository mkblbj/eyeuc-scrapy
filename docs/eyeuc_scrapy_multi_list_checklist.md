...
### 2.1 并发与风控
- [ ] AutoThrottle：初始/最小延迟 0.2–0.5s，最大 3–8s
- [ ] 并发：`CONCURRENT_REQUESTS=8~16`，域内 `4~8`，必要时降至 1–2 rps
- [ ] 随机延迟中间件（避免节律化）

### 2.2 选择器与兜底
- [ ] 关键块采用 CSS + XPath 双路；必要时轻度正则兜底
- [ ] DOM 改版监控：失败样本落盘（URL + HTML 片段）

### 2.3 错误与重试
- [ ] 分类记录网络/解析/权限错误；5xx/超时启用 RETRY（2~3 次）
- [ ] 关键指标日志：抓取量、失败率、重试率、平均响应时延

### 2.4 可重入与指纹
- [ ] `JOBDIR` 可重入验证
- [ ] 指纹键：`detail_url`；可选引入 `scrapy-deltafetch` 增量

验收标准（阶段 2）：
- [ ] 1 小时稳定长跑无崩溃，失败率 < 3%
- [ ] DOM 微调后可在 30 分钟内通过选择器兜底恢复
- [ ] 关键日志指标可用，便于回归
